...
// The build system provides the following values:
// const MEMORY_SIZE, STACK_SIZE, MAX_WORKER_THREADS;
...
const wasmUrl = 'main.wasm?'+Math.random();

// Generate shared memory
const WASM_PAGE_SIZE = 65536;
const PAGES = MEMORY_SIZE / WASM_PAGE_SIZE;
...
simWorker.onmessage = function(e) {
	let msg = e.data;
	switch (msg.type) {
		case MSG_TWEAK_RESULT: {
			tweak.ingestTweakResult(msg.tweakResult.buffer);
		} break;
		case MSG_SIM_RESULT: {
...
simWorker.postMessage({ type: MSG_INIT_WASM, wasmUrl, memory, threadId: 0, STACK_SIZE });
...
for (let i = 0; i < MAX_WORKER_THREADS; i++) {
	let worker = new Worker("sim-worker.js");
	threadWorkers[i] = worker;
	worker.postMessage({ type: MSG_INIT_WASM, wasmUrl, memory, threadId: i + 1, STACK_SIZE });
}
...
class Gfx {
	constructor(parentElement) {
		...
		this.vertBufSize = 16 * 512 * 1024;
		this.idxBufSize = 4 * 2048 * 1024;
		this.pointBufs = [];
		this.vertBufs = [];
		this.idxBufs = [];
		for (let i = 0; i < 3; i++) {
...
function update(timestamp) {
	...
	if (simResults[0]) {
		...
		// We initially cover the web page with a full screen "curtain" div and only
		// lift it after we've rendered the first frame
		if (curtainDown) {
			let curtain = document.getElementById('curtain');
			curtain.className = 'off';
			setTimeout(() => { curtain.style.display = 'none'; }, 200);
			curtainDown = false;
		}
	}
...
window.addEventListener('beforeunload', function(e) {
	simWorker.postMessage({ type: MSG_TERM_WASM });

	// Pause so we can read the cleanup logs
	// for (let i = 0; i < 4 * 1024 * 1024 * 1024; i++) {}
});

main = {
	rpc,
	private_threadWorker_FirefoxWorkerGCWorkAround: threadWorkers, // In Firefox, workers must be referenced by javascript or else they get erroneously GC'd
};
