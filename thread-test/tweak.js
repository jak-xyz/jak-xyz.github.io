...
const RangeMappingType_Exponential = 2;
	...
	constructor(parentElement, name, min, max, step, rangeToTextValueFn) {
		...
		this.formatValueForText = (value) => {
			...
			for (let i = 0; i < str.length; i++) {
				...
				if (i >= decimalIdx && i > firstSigIdx + SIG_FIG - 1) { break; }
				strBuilder.push(i < firstSigIdx + SIG_FIG ? str[i] : '0');
			}
	...
	readRangeMapping() {
		...
		switch (type) {
			...
			case RangeMappingType_Linear: {
				let preBias = this.readFloat();
				let scale = this.readFloat();
				let postBias = this.readFloat();
				fn = (x) => { return postBias + scale * (preBias + x); };
			} break;
	...
	for (let itemIdx = 0; itemIdx < itemCount; itemIdx++) {
		...
		switch (itemType) {
			...
			case Tweak_Checkboxes: {
				...
				let value0 = containers[0][label] ? containers[0][label].value : initialValue;
				let value1 = containers[1][label] ? containers[1][label].value : initialValue;
			...
			case Tweak_RangeWithTextField: {
				...
				let value0 = containers[0][label] ? containers[0][label].value : initialValue;
				let value1 = containers[1][label] ? containers[1][label].value : initialValue;
				containers[0][label] = { type: Type_Float, value: value0, offset, shift: 0, mask: 0 };
				containers[1][label] = { type: Type_Float, value: value1, offset, shift: 0, mask: 0 };
				pushValue(containerPtrs[0], containers[0][label]);
				pushValue(containerPtrs[1], containers[1][label]);
